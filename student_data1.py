...
def get_total_num_of_students()->int :
    while True:
...
def get_marks_of_sem2(number_of_subjects:int,student_details:dict)->dict :
    sub_counter= 1
    while sub_counter<= number_of_subjects:
        marks = input( "Enter the marks in Subject Number " + str(sub_counter) + ": ")
        if not marks.isdigit():
            print("Error: Specify marks of subject in digits only")
            print("Please enter the marks of this student again")
            continue
        marks = int(marks)
        if marks > MAX_SCORE:
            print("Error: Maximum score in any subject is",MAX_SCORE)
            print("Please enter the marks of this student again")
            continue
...
def get_marks_of_sem1(number_of_subjects:int,student_details:dict)->dict :
    sub_counter= 1
    while sub_counter<= number_of_subjects:
        marks = input( "Enter the marks in Subject Number " + str(sub_counter) + ": ")
        if not marks.isdigit():
    ...
    while True :
        ...
        if not number_of_subjects.isdigit():
            ...
            if number_of_subjects < MIN_NUMBER_OF_SUBJECTS:
                print("Error: Minimum number of Subjects is",MIN_NUMBER_OF_SUBJECTS)
                print("Please enter the details of this student again")
                continue
            else :
                break
...
def get_number_of_subjects_sem1()->int :
    while True :
...
def print_data(student_details:dict) :
...
if __name__ == "__main__":
    total_students = get_total_num_of_students()
    student_counter= 1
    while student_counter<= total_students:
        print("Enter the details of student number : ", student_counter)
        name =  input("Enter the Student name :")
        number_of_subjects = get_number_of_subjects_sem1()
        student_details[name] = {"sem1":[],"sem2":[]}
        student_details = get_marks_of_sem1(number_of_subjects,student_details)
        number_of_subjects = get_number_of_subjects_sem2()
        student_details = get_marks_of_sem2(number_of_subjects,student_details)  
        student_counter = student_counter + 1
    print_data(student_details)
